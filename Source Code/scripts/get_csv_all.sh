...
for file in *.o*; do
    ...
    if [[ -s $file && $file == *mtx* ]]; then
        ...
        matrix_size=$(grep "Matrix size" $file | awk '{print $3}' | sed 's/size://') # Extract the matrix size from the file
        vector_size=$(grep "Vector size" $file | awk '{print $3}' | sed 's/size://') # Extract the vector size from the file
        ...
        # Column-wise Data
        col_wise_communication_time=$(grep "Column-wise Average Communication Time" $file | awk '{print $6}') # Extract the column-wise average communication time from the file
        col_wise_computation_time=$(grep "Column-wise Average Computation Time" $file | awk '{print $6}')     # Extract the column-wise average computation time from the file
